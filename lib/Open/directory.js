...
function getCrxHeader(source) {
  return source.stream(0).then(function(stream) {
    var sourceStream = stream.pipe(PullStream());
...
function getZip64CentralDirectory(source, zip64CDL) {
  ...
  return source.stream(d64loc.offsetToStartOfCentralDirectory).then(function(stream) {
  ...
  return source.size()
    ...
    .then(function(d) {
      ...
      if (vars.numberOfRecords == 0xffff|| vars.numberOfRecords == 0xffff ||
        ...
        return source.stream(zip64CDLOffset).then(function(stream) {
      ...
      vars.extract = function(opts) {
        ...
        return vars.files.then(function(files) {
          return Promise.map(files, function(entry) {
            ...
            return entry.stream(opts.password).then(function(stream) {
      ...
      vars.files = Promise.mapSeries(Array(vars.numberOfRecords),function() {
        ...
        .then(function(comment) {
          ...
          vars.buffer = function(_password) {
            return vars.stream(_password).then(function (stream) {
